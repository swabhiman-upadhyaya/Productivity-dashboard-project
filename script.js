...
// ============================================================
// WEATHER DASHBOARD - Geolocation-based fetching
// CHANGED: Removed hardcoded city ("Bhubaneswar"). Now uses the
// browser's Geolocation API to get the user's real coordinates
// and passes them to the WeatherAPI as "q=lat,lon".
// The .Place element is also dynamically updated from the API
// response instead of being a static string in the HTML.
// ============================================================

let apiKey = "5bc62630e88241d29b195239262301"

// CHANGED: Fetches weather using latitude & longitude received
// from navigator.geolocation, so any user anywhere gets their
// actual local weather instead of always seeing Bhubaneswar.
async function FetchingWeather(lat, lon) {
    try {
        let response = await fetch(`https://api.weatherapi.com/v1/current.json?key=${apiKey}&q=${lat},${lon}`)
        let data = await response.json()

        // CHANGED: Update the Place element with the real city/region
        // returned by the API (e.g. "Kolkata, West Bengal")
        let city = data.location.name;
        let region = data.location.region;
        place.innerHTML = `${city}, (${region})`

        temperature.innerHTML = `${data.current.temp_c}°C`
        probability.innerHTML = `Weather type: ${data.current.condition.text}`
        precipitation.innerHTML = `Dew Amount: ${data.current.dewpoint_c}°C`
        humidity.innerHTML = `Humidity: ${data.current.humidity}%`
        wind.innerHTML = `Wind: ${data.current.wind_kph} km/hr`
    } catch (error) {
        // Show a friendly error if the weather fetch fails
        place.innerHTML = "Weather unavailable"
        console.error("Weather fetch failed:", error)
    }
}

// CHANGED: Use the browser Geolocation API to get the user's
// real coordinates, then pass them to FetchingWeather().
// If the user denies location permission, show a fallback message.
if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
        function (position) {
            // Success: got coordinates → fetch weather for this location
            let lat = position.coords.latitude;
            let lon = position.coords.longitude;
            FetchingWeather(lat, lon);
        },
        function (error) {
            // Error: user denied permission or geolocation failed
            place.innerHTML = "Location unavailable"
            console.warn("Geolocation error:", error.message)
        }
    );
} else {
    // Browser does not support Geolocation
    place.innerHTML = "Geolocation not supported"
